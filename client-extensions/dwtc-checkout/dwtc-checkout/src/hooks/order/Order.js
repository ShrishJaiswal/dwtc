...
export function useCommerceOrderFetch(refreshFlag){
    ...
    useEffect(()=>{
        try {
            if (Liferay?.CommerceContext?.order) {
              setCart(Liferay.CommerceContext.order);
            } else {
                setError(ErrorConstants.NO_OPEN_ORDER_FOUND);
            }
        } catch (err) {
        console.error("Exception while fetching commerce order :: " + err);
        } finally {
            setLoading(false);
        }
    }, [refreshFlag]);
